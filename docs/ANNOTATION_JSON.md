...
Single-video annotation JSON v2/v3 can be imported through **Import annotations → Frameinsight annotations — JSON** onto the matching original video. Preview checks dimensions, frame count and available source hashes. Current boxes, IDs when unused, class styles, provenance and deleted intervals are restored as a new undoable import. Old undo history, proposal references, review confirmations and validation proofs are not restored. For full editable recovery, keep a **Project backup ZIP** plus original videos. Native backup schema v2 supports named classes and restore accepts both v1 and v2. Legacy CVAT/YOLO/MOT profiles reject dynamic-class data rather than silently omitting it; use the v3 JSON for conversion.
