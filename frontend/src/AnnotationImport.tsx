...
export function AnnotationImport({onClose,onBusy}:{onClose:()=>void;onBusy:(busy:boolean)=>void}){
 const s=useStore(),[format,setFormat]=useState('frameinsight'),[file,setFile]=useState<File|null>(null),[frameBase,setFrameBase]=useState(0),[coordinateBase,setCoordinateBase]=useState(0),[names,setNames]=useState(''),[clip,setClip]=useState(false),[preview,setPreview]=useState<Preview|null>(null),[busy,setBusy]=useState(false),[error,setError]=useState(''),[applied,setApplied]=useState(false);
 ...
 return <div className="annotation-import"><p>Import labels for <strong>{s.project?.videos[s.videoId]?.name}</strong>. Existing tracks are kept. Review the result before training.</p>{format==='frameinsight'&&<p className="form-help">Use the original video and import into a new project to keep all original IDs. Existing IDs are remapped, never silently joined. Saved boxes, keyframes and deleted ranges are retained; old undo history and validation are not.</p>}<fieldset disabled={busy||applied} onChange={reset}><label>Format<NativeSelect aria-label="Annotation format" value={format} onChange={e=>{reset();setFormat(e.target.value);setFile(null);setFrameBase(e.target.value==='mot'?1:0)}}><option value="frameinsight">Frameinsight annotations — JSON</option><option value="yolo">YOLO detection — 5 columns, no track IDs</option><option value="yolo_tracks">YOLO with track IDs — 6 columns</option><option value="mot">MOT 1.1 ground truth</option></NativeSelect></label><label>Annotation file<Input aria-label="Annotation file" key={format} type="file" accept={format==='frameinsight'?'.json':'.zip,.txt'} onChange={e=>setFile(e.target.files?.[0]||null)}/></label>{format!=='frameinsight'&&<><label>First source frame number<NativeSelect aria-label="First source frame number" value={frameBase} onChange={e=>{reset();setFrameBase(Number(e.target.value))}}><option value={0}>0 → app frame 0</option><option value={1}>1 → app frame 0</option></NativeSelect></label>{format==='mot'&&<label>Box coordinates<NativeSelect aria-label="Box coordinate base" value={coordinateBase} onChange={e=>{reset();setCoordinateBase(Number(e.target.value))}}><option value={0}>Start at 0 (CVAT exports)</option><option value={1}>Start at 1 (MOTChallenge)</option></NativeSelect></label>}<details><summary>Class names and image boundaries</summary><label>Override class names — one per line<Textarea aria-label="Import class names" rows={4} placeholder="Leave empty to use the archive’s class list" value={names} onChange={e=>setNames(e.target.value)}/></label><p className="form-help">List names in source ID order: YOLO starts at 0; MOT starts at 1. Without an archive class list, YOLO uses project classes and MOT uses standard MOT labels.</p><label><Checkbox checked={clip} onCheckedChange={checked=>{reset();setClip(checked===true)}}/>Clip boxes that extend beyond the image</label></details></>}</fieldset>{error&&<p role="alert" className="error">{error}</p>}{preview&&<section className="import-preview" aria-label="Import preview"><h3>{preview.summary.boxes} boxes · {preview.summary.tracks} tracks</h3><p>{preview.summary.frames} labeled frames, from app frame {preview.summary.first_frame} to {preview.summary.last_frame}. Frames without labels stay Hidden.</p>{preview.warnings.map(w=><p className="import-warning" key={w}>{w}</p>)}<details><summary>Track ID mapping</summary><div className="import-mapping">{preview.mapping.slice(0,200).map(m=><div key={m.source}>Source {m.source} → Track {m.track_id}</div>)}</div>{preview.mapping.length>200&&<p>Showing the first 200 mappings.</p>}</details><p>Adding labels does not confirm that their identities or locations are correct. Inspect playback before finishing.</p></section>}<div className="button-row">{!applied&&<Button disabled={busy||!file} onClick={()=>void inspect()}>{busy?'Working…':preview?'Preview again':'Preview import'}</Button>}{preview&&<Button className="primary" disabled={busy} onClick={()=>void apply()}>{applied?'Retry saving import':'Add annotations'}</Button>}</div></div>
}
