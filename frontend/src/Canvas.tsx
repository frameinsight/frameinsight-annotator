...
const displayedGeometries:Geometry[]=VISIBLE_ONLY?['person_visible']:geometries;
...
export const EditorCanvas=forwardRef<CanvasHandle,{proposals:Proposal[];showGhosts:boolean;showProposals:boolean}>(({proposals,showGhosts,showProposals},ref)=>{
 ...
 function finish(){
  ...
  const hit=boxEntries.find(b=>edgeHit(b.box,g.start,7/view.scale));
 ...
 function down(e:React.PointerEvent){
  ...
  else{const hit=boxEntries.find(b=>edgeHit(b.box,p,7/view.scale));
 ...
 return <div className="canvas-host" ref={host} tabIndex={0} role="application" aria-label="Video annotation canvas" data-testid="canvas" data-frame={ready?frame:'loading'} data-scale={view.scale} data-offset-x={view.x} data-offset-y={view.y} onPointerDown={down} onPointerMove={move} onPointerUp={finish} onPointerCancel={()=>{gesture.current=null;setPreview(null)}} onWheel={wheel} style={{cursor}}>
  {ready?<Stage width={size.w} height={size.h} listening={false}><Layer listening={false}><Group x={view.x} y={view.y} scaleX={view.scale} scaleY={view.scale}><KImage image={displayedImage} width={video.width} height={video.height}/></Group></Layer><Layer listening={false}><Group x={view.x} y={view.y} scaleX={view.scale} scaleY={view.scale}>
   {ghost&&displayedGeometries.map(g=>ghost[g]&&drawBox(ghost[g]!,g,'ghost'+g,'',false,.22,true,project?.state.identities[ghost.identity_uuid]?.color||colors[g]))}
   ...
   {boxEntries.slice().reverse().map(({o,g,box})=>{const selected=o.identity_uuid===activeId&&g===geometry;const editing=gesture.current?.activeId===o.identity_uuid&&gesture.current.geometry===g;const linkedPreview=preview&&o.geometry_link==='equal'&&gesture.current?.geometry==='person_ext'&&gesture.current.activeId===o.identity_uuid;const b=editing&&preview?preview:linkedPreview?preview:box;const person=project!.state.identities[o.identity_uuid];return drawBox(b,g,o.id+g,`${VISIBLE_ONLY?(person?.class_name||'Person'):g==='person_ext'?'A':'B'} · ${person?.person_id??person?.name??'Draft'}${!VISIBLE_ONLY&&o.review_state==='approved'?' ✓':''}`,selected,1,false,person?.color||colors[g]);})}
   {preview&&gesture.current?.kind==='draw'&&!active?.[gesture.current.geometry]&&drawBox(preview,gesture.current.geometry,'drawing',VISIBLE_ONLY?'Person':gesture.current.geometry==='person_ext'?'A':'B',true,1,false,project?.state.identities[activeId]?.color||colors[geometry])}
