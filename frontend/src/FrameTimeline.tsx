...
export function FrameTimeline({bins,active,label,color,frame,count,onSeek}:{bins:Bin[];active:boolean;label:string;color:string;frame:number;count:number;onSeek:(frame:number)=>void}){
  const state=(bin:Bin)=>!active?'No track selected':bin.gap?(bin.manual||bin.generated?'Present and Hidden in this range':'Hidden'):bin.manual||bin.generated?'Present':'No box';
  return <section className="timeline presence-timeline" aria-label="Annotation timeline">
    <div className="timeline-title"><strong title={label}>{label}</strong><div className="legend"><span><i style={{background:color}}/>Present</span><span><i className="hidden-color"/>Hidden</span><span><i className="empty-color"/>No box</span></div><span className="review-progress">{count.toLocaleString()} frames</span></div>
    <div className="timeline-track" role="group" aria-label="Frame ranges">{bins.map((bin,index)=>{
      const present=active&&(bin.manual||bin.generated),hidden=active&&bin.gap;
      const current=frame>=bin.start&&frame<bin.end;
      return <button key={index} aria-label={`Frames ${bin.start}–${bin.end-1}: ${state(bin)}`} title={`Frames ${bin.start}–${bin.end-1}: ${state(bin)}${hidden?' · Restore range to fill again':''}`} aria-current={current?'step':undefined} className={(hidden?'gap ':present?'present ':'empty ')+(current?'current':'')} style={{background:present&&hidden?`repeating-linear-gradient(135deg,${color} 0 5px,#ef4444 5px 10px)`:hidden?'#ef4444':present?color:'#303030'}} onClick={()=>onSeek(bin.start)}/>;
    })}</div>
