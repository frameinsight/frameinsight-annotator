...
export function FrameTimeline({bins,active,label,color,frame,count,onSeek,controls}:{bins:Bin[];active:boolean;label:string;color:string;frame:number;count:number;onSeek:(frame:number)=>void;controls?:ReactNode}){
  ...
  return <section className="timeline presence-timeline" aria-label="Annotation timeline">
    <div className="timeline-title" data-testid="timeline-header">
      ...
      {controls}
      <span className="review-progress" data-testid="timeline-trailing">{count.toLocaleString()} frames</span>
    </div>
