...
export function interpolatePerson(d:Domain,videoId:string,identity:string,times:(number|null)[]=[],changedFrame?:number,selection:boolean|Geometry=true){
 ...
 for(const g of selected){
  ...
  const anchors=observations.filter(o=>(o[g]||o.provenance[g])&&(o.review_state==='approved'||!isGeometryGenerated(o,g)));
  ...
  for(let i=1;i<anchors.length;i++){
   ...
   for(let f=start+1;f<end;f++){
    const old=existing.get(f);if(old&&(old.review_state==='approved'||((old[g]||old.provenance[g])&&!isGeometryGenerated(old,g))))continue;
