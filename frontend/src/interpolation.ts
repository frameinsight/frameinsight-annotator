import {type Box,type Domain,type Observation,type Geometry,geometries,emptyObservation,getBox,setBox,boxKeys,identityGeometryKeys} from './types';
...
export function markCorrected(o:Observation,geometry?:Geometry){
 for(const g of geometry?[geometry]:[...new Set([...boxKeys(o),...Object.keys(o.provenance)])])if(['model_track','interpolated','copied','copied_track'].includes(o.provenance[g]?.origin||''))o.provenance[g]={...o.provenance[g]!,human_corrected:true};
}
...
export function interpolatePerson(d:Domain,videoId:string,identity:string,times:(number|null)[]=[],changedFrame?:number,selection:boolean|Geometry=true){
 const selected=typeof selection==='string'?[selection]:selection?['person_visible'] as Geometry[]:identityGeometryKeys(d,identity,videoId);
 ...
 for(const g of selected){
  ...
  const anchors=observations.filter(o=>(getBox(o,g)||o.provenance[g])&&(o.review_state==='approved'||!isGeometryGenerated(o,g)));
  ...
  for(let i=1;i<anchors.length;i++){
   ...
   if(end-start<2||left.segment_id!==right.segment_id||!getBox(left,g)||!getBox(right,g))continue;
   ...
   for(let f=start+1;f<end;f++){
    const old=existing.get(f);if(old&&(old.review_state==='approved'||((getBox(old,g)||old.provenance[g])&&!isGeometryGenerated(old,g))))continue;
    ...
    setBox(o,g,lerp(getBox(left,g)!,getBox(right,g)!,t));o.geometry_link='independent';
