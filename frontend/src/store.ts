...
export const useStore=create<Store>((set,get)=>({
 ...
 newPerson:()=>{
  ...
  const id=uuid(),seg=uuid(),numbers=new Set(Object.values(s.project!.state.identities).map(p=>p.person_id));let number=1;while(numbers.has(number))number++;const ok=s.commit('New person',d=>{d.identities[id]={id,person_id:number,name:'Person '+number,class_name:s.project?.classes?.[0]||'Person',box_styles:{[s.geometry]:{class_name:s.geometry==='person_ext'?'person_extended':s.project?.classes?.[0]||'Person',color:classColor(s.project,s.geometry==='person_ext'?'person_extended':s.project?.classes?.[0]||'Person')}}};d.segments[seg]={id:seg,video_id:s.videoId,identity_uuid:id,start:s.frame,end:null,status:'verified'};});if(ok)set({activeId:id});
 },
