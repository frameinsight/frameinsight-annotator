...
export const useStore=create<Store>((set,get)=>({
 ...
 restoreRange:(start,end,mode,operations)=>{const s=get();if(!s.project)return false;let result:RestorePreview|undefined;const ok=s.commit(mode==='original'?'Recover original deleted boxes':'Restore deleted range with interpolation',d=>{result=restoreRecovery(d,s.videoId,s.activeId,start,end,s.project!.videos[s.videoId].frame_count,s.geometry,mode,operations??s.history,s.frameTimes[s.videoId]);});if(ok&&result)s.toast(`${result.restoredBoxes} boxes restored; ${result.keptBoxes} existing boxes kept.${result.remainingBlockedFrames?' '+result.remainingBlockedFrames+' deleted frames still need boundary boxes.':''} Ctrl+Z undoes this recovery.`);return ok;},
 ...
 commit:(label,fn)=>{
  ...
  if(changes.length>50000){s.toast('This action changes more than 50,000 items. Choose a smaller frame range. Nothing was changed.');return false;}
 ...
 newPerson:()=>{
  ...
  const selectedName=boxStyle(s.project?.state.identities[s.activeId],s.geometry).class_name;
  const color=unusedBoxColor([...Object.values(s.project?.class_colors||{}),...Object.values(s.project!.state.identities).flatMap(p=>[p.color||trackColor(p.person_id,p.id),...Object.values(p.box_styles||{}).map(style=>style?.color||'')])]);
  const id=uuid(),seg=uuid(),name=s.project?.classes?.includes(selectedName)?selectedName:(s.project?.classes?.[0]||'Person'),geometry=classKey(name),numbers=new Set(Object.values(s.project!.state.identities).map(p=>p.person_id));let number=1;while(numbers.has(number))number++;const ok=s.commit('New track',d=>{d.identities[id]={id,person_id:number,name:'Track '+number,class_name:name,color,box_styles:{[geometry]:{class_name:name,color}}};d.segments[seg]={id:seg,video_id:s.videoId,identity_uuid:id,start:s.frame,end:null,status:'verified'};});if(ok)set({activeId:id,geometry,unregisteredTracks:{...s.unregisteredTracks,[id]:true}});
 },
 ...
 setBox:(geometry,box,proposal,context)=>{
  ...
  s.editObservation('Edit '+geometry,o=>{
   ...
   o.geometry_link='independent';putBox(o,geometry,box);
 ...
 equal:()=>{
  const s=get();if(s.geometry.startsWith('class:'))return s.toast('Use Copy to class and choose a target class.');const o=currentObservation(s.project,s.videoId,s.frame,s.activeId);if(!o)return s.toast('Draw a box first');const box=getBox(o,s.geometry)||o.person_ext||o.person_visible;if(!box)return s.toast('Draw a box first');
  s.editObservation('Clear person · A = B',n=>{n.person_ext=[...box];n.person_visible=[...box];n.geometry_link='equal';n.full_quality='observed';n.occluded=false;n.provenance.person_ext=n.provenance.person_ext||{origin:'manual',proposal_id:null,human_corrected:false};n.provenance.person_visible={origin:'manual',proposal_id:null,human_corrected:false};});s.toast((getBox(o,s.geometry)?s.geometry==='person_ext'?'A':'B':o.person_ext?'A':'B')+' is authoritative. A = B; review truncation before approval.');
 },
 copyPrevious:()=>{
 ...
 assignPerson:(target,personId,className,color,chosenGeometry)=>{const s=get();if(target!==s.activeId||Object.values(s.project!.state.identities).some(p=>p.id!==s.activeId&&p.person_id===personId)){s.toast('That ID belongs to another track. Choose a different ID.');return false;}const g=chosenGeometry||geometryForClass(s.project?.state.identities[target],className),sourceGeometry=s.project&&legacyExtended(s.project.state.identities[s.activeId])?'person_ext':s.geometry;const reconnect=s.activeId!==target&&s.project&&Object.values(s.project.state.observations).some(o=>o.identity_uuid===target&&getBox(o,g));const ok=s.commit('Assign existing track / class / color',d=>{assignPersonInDomain(d,s.activeId,target,s.videoId,s.frame,personId,className,color,g,sourceGeometry);if(reconnect)interpolatePerson(d,s.videoId,target,s.frameTimes[s.videoId],s.frame,geometryForClass(d.identities[target],className))});if(ok){set({unregisteredTracks:{...get().unregisteredTracks,[s.activeId]:false}});get().selectPerson(target);set({geometry:geometryForClass(get().project!.state.identities[target],className)});}return ok;},
 rename:(id,n)=>{const s=get();if(n!==null&&(!Number.isSafeInteger(n)||n<=0)){s.toast('Person ID must be a positive integer');return;}s.commit('Rename person ID',d=>{if(n!==null&&Object.values(d.identities).some(i=>i.id!==id&&i.person_id===n))throw new Error('That ID is already assigned. Choose a different ID.');d.identities[id].person_id=n;});},
}));
