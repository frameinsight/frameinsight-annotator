...
async function validatedExport(request:any){
 ...
 await expect.poll(async()=> (await (await request.get('/api/jobs/'+review)).json()).status,{timeout:30000}).toBe('completed');
 const checked=await request.post(`/api/videos/${videoId}/validate`,{data:{review_job_id:review,revision,visual_confirmed:true,coverage:'selected_people'}});expect(checked.ok()).toBeTruthy();const report=await checked.json();expect(report.passed,JSON.stringify(report.errors)).toBe(true);
 const response=await request.post(`/api/projects/${projectId}/exports`,{data:{format:'annotations_json',video_id:videoId,revision,review_job_id:review,validation_id:report.validation_id}});expect(response.ok()).toBeTruthy();const job=(await response.json()).id;
...
test('finish reviews and validates before exporting annotations only; edits reopen finished work',async({page,request})=>{
 await page.getByTestId('canvas').press('n');await drag(page,[100,80],[200,300]);await page.getByRole('button',{name:'Finish',exact:true}).click();await expect(page.getByRole('link',{name:'Download annotations (.json)'})).toHaveCount(0);
 await page.getByRole('button',{name:'Prepare review video',exact:true}).click();await expect(page.getByTestId('review-video')).toBeVisible({timeout:30000});await expect(page.getByLabel('Visual review complete')).toBeEnabled();await expect(page.getByRole('button',{name:'Run annotation validation'})).toBeDisabled();
...
test('restore deleted range preserves new endpoints, supports exact undo, and recovers originals in a fresh browser',async({page,request,browser})=>{
 ...
 await page.getByRole('button',{name:'Restore deleted range',exact:true}).first().click();await page.getByLabel('First frame to restore').fill('8');await page.getByLabel('Last frame to restore').fill('12');await expect(page.getByRole('dialog')).toContainText('3 boxes to restore');await expect(page.getByRole('dialog')).toContainText('2 existing boxes kept');
 ...
 const restored=await state(request);expect(at(restored,10).person_visible[0]).toBeCloseTo(160,0);expect(at(restored,8)).toEqual(at({state:blocked},8));expect(at(restored,12)).toEqual(at({state:blocked},12));expect(Object.values(restored.state.intervals)).toHaveLength(0);
 ...
 try{
  ...
  const recovered=await state(request);expect(at(recovered,10).person_visible).toEqual(at({state:original},10).person_visible);expect(at(recovered,8)).toEqual(at({state:blocked},8));expect(at(recovered,12)).toEqual(at({state:blocked},12));expect(Object.values(recovered.state.intervals)).toHaveLength(0);
...
test('missing boxes are automatically not visible; Delete saves absence and drawing restores it without G or H',async({page,request})=>{
 await page.getByTestId('canvas').press('n');await expect(page.locator('.person').filter({hasText:'Person 1'})).toContainText('No box yet');
 ...
 await page.getByTestId('canvas').press('Control+z');await saved(page);expect(at(await state(request),5)).toBeUndefined();await expect(page.locator('.gap-banner')).toContainText('Interpolation paused');await page.keyboard.press('Control+Shift+z');await saved(page);expect(at(await state(request),5).person_visible).not.toBeNull();
 await page.getByLabel('Go to frame').fill('15');await ready(page,15);await expect(page.locator('.gap-banner')).toContainText('No visible box yet');await drag(page,[230,80],[330,300]);await saved(page);expect(at(await state(request),15).person_visible).not.toBeNull();
 await reopen(page);await ready(page,15);await page.getByLabel('Go to frame').fill('23');await ready(page,23);await expect(page.locator('.gap-banner')).toContainText('No visible box yet');
});
...
test('copy Visible to Extended selects a separate colored box; resize, undo, reload and JSON retain the same ID',async({page,request})=>{
 ...
 const exportId=await validatedExport(request);const doc=await (await request.get('/api/exports/'+exportId)).json();const rows=doc.annotation_index.filter((r:any)=>r.frame_index===0);expect(rows).toHaveLength(2);expect(rows.map((r:any)=>r.person_id)).toEqual([7,7]);expect(rows.map((r:any)=>r.color)).toEqual(['#22d3ee','#fb923c']);expect(rows.map((r:any)=>r.class_name)).toEqual(['person_visible','person_extended']);
