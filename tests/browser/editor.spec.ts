...
test('hidden range deletes interpolated boxes, preserves other people, and survives undo, redo, export and reload',async({page,request})=>{
 ...
 await reopen(page);await ready(page,13);await page.getByLabel('Go to frame').fill('10');await ready(page,10);await expect(page.locator('.gap-banner')).toContainText('Not visible — no box on this frame');
