...
test('real-clip navigation measurement records actual worker status',async({page,request})=>{
 ...
 const coldStart=Date.now();await page.getByTestId('open-video-'+vid).click();await page.getByLabel('Go to frame').fill('0');await expect(page.getByTestId('canvas')).toHaveAttribute('data-frame','0');const coldLoadMs=Date.now()-coldStart;
