...
test('project workflow, class order, tracked import, undo, reload and video isolation',async({page,request})=>{
 ...
 const canvas=await page.getByTestId('canvas').boundingBox(),tracks=await page.locator('.people-panel').boundingBox(),transport=await page.locator('.transport').boundingBox();expect(tracks!.x).toBeGreaterThan(canvas!.x);expect(transport!.y).toBeLessThan(canvas!.y);
 ...
 await page.locator('.project-switch').click();await expect(page.locator('.video-card')).toHaveCount(2);await page.getByTestId('open-video-'+vid).click();await expect(track7).toBeVisible();
});
...
test('MOT import applies frame/coordinate bases, stays editable and rejects invalid replacement previews',async({page,request})=>{
 ...
 await openImport(page,'mot','gt.txt','1,7,100,80,100,220,1,1,1\n11,7,200,80,100,220,1,1,.5');await page.getByRole('button',{name:'Add annotations',exact:true}).click();await expect(page.getByRole('dialog')).toHaveCount(0);
 ...
 await page.getByRole('button',{name:'Import annotations',exact:true}).click();await page.getByLabel('Annotation file',{exact:true}).setInputFiles({name:'99999.txt',mimeType:'text/plain',buffer:Buffer.from('0 .5 .5 .2 .2')});await page.getByRole('button',{name:'Preview import',exact:true}).click();await expect(page.getByRole('alert')).toContainText('outside');await expect(page.getByRole('button',{name:'Add annotations',exact:true})).toHaveCount(0);
});
